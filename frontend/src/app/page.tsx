...
function UsageTab() {
  ...
  const imageUsed = txs.filter((t) => t.transaction_type === 'deduct' && t.amount >= 100).length;
  const textUsed = txs.filter((t) => t.transaction_type === 'deduct' && t.amount < 100).length;
  ...
  return (
    <div>
        ...
        <div className="col-span-2 bg-white border border-gray-200 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-4xl font-semibold text-gray-900 tracking-tight tabular-nums">{creditsInfo.balance.toLocaleString()}</p>
            </div>
      ...
      <p className="text-xs text-gray-400 mt-4">
        初回登録時に4,500クレジットが付与されます。処理開始時にクレジットが消費されます。
      </p>
    </div>
  );
}
      ...
      subsections: [
        {
          title: '4-1. クレジットの付与',
          items: [
            '新規アカウント登録時に4,500クレジットが無償で付与されます。',
        ...
        },
        {
          title: '4-6. クレジットの購入',
          items: [
            'クレジットはStripe決済（クレジットカード）を通じて購入できます。',
            '購入したクレジットに有効期限はありません（サービス終了時を除く）。',
            '決済完了後のクレジット購入の返金には原則対応しません。ただし、システム障害等により正常に付与されなかった場合は個別対応します。',
          ],
        },
        {
          title: '4-7. クレジットの譲渡・換金',
          items: [
...
export default function HomePage() {
  ...
  const [paymentNotification, setPaymentNotification] = useState<'success' | 'cancelled' | null>(null);
  ...
  useEffect(() => {
    ...
    if (payment === 'success') {
      ...
      setPaymentNotification('cancelled');
      setActiveTab('usage');
      window.history.replaceState({}, '', '/');
      setTimeout(() => setPaymentNotification(null), 5000);
    }
    ...
    <div className="min-h-screen bg-[#fafafa] flex">
      ...
      <aside className="w-[220px] bg-white border-r border-gray-200 flex flex-col flex-shrink-0">
        ...
        <div className="px-3 mb-2 space-y-1.5">
          ...
          <button
            onClick={() => setActiveTab('usage')}
            className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 text-[11px] font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg transition-colors"
          >
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-8 py-8">
          ...
          {paymentNotification && (
            <div className={cn(
              'mb-6 flex items-center gap-3 px-5 py-4 rounded-xl border',
              ...
              {paymentNotification === 'success' ? (
                <>
                  ...
                  <div>
                    <p className="text-sm font-medium text-emerald-800">決済が完了しました</p>
                    <p className="text-xs text-emerald-600 mt-0.5">100,000クレジットが追加されました。残高に反映されるまで数秒かかる場合があります。</p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <p className="text-sm font-medium text-amber-800">決済がキャンセルされました</p>
                    <p className="text-xs text-amber-600 mt-0.5">クレジットは消費されていません。</p>
                  </div>
                </>
              )}
              <button

...
import {
  ...
  Settings2, Wallet, UserCog, Clock, Image, Type,
  TrendingDown, TrendingUp, Hash,
} from 'lucide-react';
...
function UserDetailPanel({ userId, onBack }: { userId: string; onBack: () => void }) {
  ...
  return (
    <div className="space-y-5">
      {/* ===== Hero Header ===== */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-[#0d0d0d] via-[#1a1a2e] to-[#16213e] p-6">
        {/* Background pattern */}
        ...
        <div className="relative flex items-start gap-4">
          <button onClick={onBack} className="mt-0.5 p-1.5 hover:bg-white/10 rounded-lg transition-colors">
            <ChevronLeft className="w-5 h-5 text-white/60" />
          </button>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <h3 className="text-lg font-semibold text-white truncate">{detail.user.email}</h3>
              {isBanned ? (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-red-500/20 text-red-300 border border-red-500/30">
            ...
            <div className="flex items-center gap-3 text-xs text-white/40">
              <span className="flex items-center gap-1">
                ...
                {new Date(detail.user.created_at).toLocaleDateString('ja-JP')} 登録
              </span>
              <span className="flex items-center gap-1">
                <Hash className="w-3 h-3" />
                {detail.user.id.substring(0, 8)}
              </span>
            </div>
          </div>
          <div className="text-right shrink-0">
            ...
            <div className="text-xs text-white/40 font-medium">credits</div>
          </div>
        </div>
        ...
        <div className="relative mt-5 grid grid-cols-3 gap-3">
          {[
            { icon: Image, label: '画像生成', value: imageCount, color: 'text-purple-300' },
            { icon: Type, label: 'テキスト', value: textCount, color: 'text-blue-300' },
            { icon: TrendingDown, label: '総消費', value: `${totalSpent}cr`, color: 'text-amber-300' },
          ].map((stat) => (
            <div key={stat.label} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-white/[0.06] border border-white/[0.08]">
              <stat.icon className={cn('w-4 h-4', stat.color)} />
              <div>
                <div className="text-sm font-semibold text-white tabular-nums">{stat.value}</div>
      ...
      {message && (
        <div className={cn(
          'flex items-center gap-2.5 px-4 py-3 rounded-xl text-sm font-medium shadow-sm',
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center gap-2 px-5 py-3.5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
            <Wallet className="w-4 h-4 text-gray-400" />
            <h4 className="text-sm font-semibold text-gray-800">クレジット操作</h4>
          </div>
          <div className="p-5 space-y-3.5">
            <div>
              <label className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-1.5 block">数量</label>
              <input
                type="number"
                placeholder="例: 50"
                value={adjustAmount}
                onChange={(e) => setAdjustAmount(e.target.value)}
                className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-300 transition-all bg-gray-50/50"
                min="1"
              />
            </div>
            <div>
              <label className="text-[11px] font-medium text-gray-400 uppercase tracking-wider mb-1.5 block">理由（任意）</label>
              <input
                type="text"
            ...
            <div className="flex gap-2.5 pt-1">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-medium bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none transition-all shadow-sm shadow-emerald-500/20"
              >
                <TrendingUp className="w-3.5 h-3.5" /> 付与
              </button>
              <button
                onClick={() => handleAdjust(false)}
                disabled={actionLoading || !adjustAmount}
                className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-medium bg-gray-800 text-white rounded-xl hover:bg-gray-900 active:scale-[0.98] disabled:opacity-40 disabled:pointer-events-none transition-all shadow-sm"
              >
                <TrendingDown className="w-3.5 h-3.5" /> 減算
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
          <div className="flex items-center gap-2 px-5 py-3.5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
            ...
            <h4 className="text-sm font-semibold text-gray-800">アカウント管理</h4>
          </div>
          <div className="p-5 space-y-3">
            ...
            {isBanned ? (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium bg-emerald-500 text-white rounded-xl hover:bg-emerald-600 active:scale-[0.98] disabled:opacity-40 transition-all shadow-sm shadow-emerald-500/20"
              >
                <ShieldCheck className="w-4 h-4" /> BAN解除する
              </button>
            ) : (
              <button
                onClick={() => handleBan(true)}
                disabled={actionLoading}
                className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium bg-red-500 text-white rounded-xl hover:bg-red-600 active:scale-[0.98] disabled:opacity-40 transition-all shadow-sm shadow-red-500/20"
              >
                <Ban className="w-4 h-4" /> ユーザーをBAN
              </button>
            )}
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              <span className="text-[10px] text-gray-300 uppercase tracking-widest">password</span>
              <div className="flex-1 border-t border-gray-100" />
            </div>
            ...
            {showPasswordForm ? (
              <div className="space-y-2.5">
                <input
                  ...
                  className="w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-300 transition-all bg-gray-50/50 font-mono"
                />
                <div className="flex gap-2">
                  <button
                    onClick={handleResetPassword}
                    disabled={actionLoading || !newPassword}
                    className="flex-1 flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-medium bg-[#0d0d0d] text-white rounded-xl hover:bg-gray-800 active:scale-[0.98] disabled:opacity-40 transition-all"
                  >
                    <Check className="w-3.5 h-3.5" /> 変更を確定
                  </button>
                  <button
                    onClick={() => { setShowPasswordForm(false); setNewPassword(''); }}
                    className="px-3 py-2.5 text-sm text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-xl transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium border border-gray-200 text-gray-600 rounded-xl hover:bg-gray-50 hover:border-gray-300 active:scale-[0.98] transition-all"
              >
                <Key className="w-4 h-4" /> パスワード強制変更
              </button>
            )}
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
          <div className="flex items-center gap-2">
            <Wallet className="w-4 h-4 text-gray-400" />
            <h4 className="text-sm font-semibold text-gray-800">クレジット履歴</h4>
          </div>
          <span className="text-[11px] text-gray-400 tabular-nums">{detail.transactions.length}件</span>
        </div>
        <div className="divide-y divide-gray-100/70 max-h-[280px] overflow-y-auto">
          {detail.transactions.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-gray-300">
              <Wallet className="w-8 h-8 mb-2 opacity-40" />
              <span className="text-xs">取引履歴はまだありません</span>
            </div>
          ) : (
            detail.transactions.map((tx, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/50 transition-colors">
                <div className={cn(
                  ...
                  {tx.transaction_type === 'deduct' ? <TrendingDown className="w-4 h-4" /> : <TrendingUp className="w-4 h-4" />}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-gray-800 truncate">{tx.description || tx.transaction_type}</p>
                ...
                <div className="text-right shrink-0">
                  <p className={cn(
                    ...
                    tx.transaction_type === 'deduct' ? 'text-red-500' : 'text-emerald-500'
                  )}>
                    {tx.transaction_type === 'deduct' ? '-' : '+'}{tx.amount}
                  </p>
                  <p className="text-[11px] text-gray-400 tabular-nums">残高 {tx.balance_after}</p>
                </div>
              </div>
            ))
          )}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm">
        <div className="flex items-center justify-between px-5 py-3.5 bg-gradient-to-r from-gray-50 to-white border-b border-gray-100">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-semibold text-gray-800">API利用履歴</h4>
          </div>
        ...
        <div className="divide-y divide-gray-100/70 max-h-[280px] overflow-y-auto">
          {detail.requests.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-gray-300">
              ...
              <span className="text-xs">API利用履歴はまだありません</span>
            </div>
          ) : (
            detail.requests.map((req, i) => (
              <div key={i} className="flex items-center gap-3 px-5 py-3 hover:bg-gray-50/50 transition-colors">
                <div className={cn(
                  'w-8 h-8 rounded-lg flex items-center justify-center shrink-0',
                  req.request_type === 'image_generation'
                    ? 'bg-purple-50 text-purple-500'
                    : 'bg-blue-50 text-blue-500'
                )}>
                  {req.request_type === 'image_generation' ? <Image className="w-4 h-4" /> : <Type className="w-4 h-4" />}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-gray-800">
                      {req.request_type === 'image_generation' ? '画像生成' : 'テキスト生成'}
                    </span>
                    <span className={cn(
                      'inline-flex px-1.5 py-0.5 rounded-md text-[10px] font-semibold',
                ...
                <div className="text-right shrink-0">
                  <span className="text-sm font-semibold text-gray-700 tabular-nums">{req.cost}cr</span>
                </div>
              </div>
            ))
          )}
